...
function App() {
  ...
  const MAX_HISTORY = 4; // limit to 4 commands

  useEffect(() => {
    const stored = localStorage.getItem('commandHistory');
    if (stored) setHistory(JSON.parse(stored).slice(-MAX_HISTORY));
  }, []);
  ...
  const handleSend = () => {
    ...
    const newHistory = [entry, ...history].slice(0, MAX_HISTORY); // keep newest MAX_HISTORY
    setHistory(newHistory);
    localStorage.setItem('commandHistory', JSON.stringify(newHistory));

    fetch('https://hooks.zapier.com/hooks/catch/24843528/u9rlzn9/', {
  ...
  return (
    <div className="chat-ui">
      <div className={`agent-icon${shaking ? ' shake' : ''}`}>
        {/* ✅ Replaced FontAwesome robot with new logo from public folder */}
        <img
      ...
      <footer className="footer">
        ...
        <a href="https://www.instagram.com/00_.siddhu._00/" target="_blank" rel="noopener noreferrer">
          Siddhant Mohan Jha
        </a>
      </footer>
    </div>
